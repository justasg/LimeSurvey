                    ...
                    <td align='center'>
                        ...
                        <?php if (hasSurveyPermission($iSurveyId,'responses','update'))
                            ...
                            <input style='height: 16; width: 16px; font-size: 8; font-family: verdana' type='image' src='<?php echo $sImageURL; ?>edit_16.png'
                                title='<?php $clang->eT('Edit entry'); ?>' onclick="window.location='<?php echo $this->createUrl("admin/dataentry/sa/editdata/subaction/edit/surveyid/{$iSurveyId}/id/{$oResult->srid}"); ?>'" />
                            <?php }
                            ...
                            <input style='height: 16; width: 16px; font-size: 8; font-family: verdana' type='image' src='<?php echo $sImageURL; ?>token_delete.png'
                                title='<?php $clang->eT('Delete entry'); ?>' onclick="if (confirm('<?php $clang->eT('Are you sure you want to delete this entry?', 'js'); ?>')) { window.location='<?php echo $this->createUrl("admin/saved/delete/surveyid/{$iSurveyId}/srid/{$oResult->srid}/scid/{$oResult->scid}"); ?>'; }" />
                            <?php } ?>
